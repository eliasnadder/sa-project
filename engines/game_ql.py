...
from engines.sticks import throw_sticks
from players.player_rl import QLearningAI  # الـ AI الذي يتعلم ويحدث weights
        ...
        while not self.game_over:
            ...
            while turn_active:
                ...
                if not valid_moves:
                    ...
                    # تحديث AI أثناء اللعب (إذا كان AI)
                    if type(self.ai_player == QLearningAI):
                        if self.ai_player and self.current_player == PlayerType.OPPONENT:
                            # افترض reward بسيط لكل حركة ناجحة (+10)
                            self.ai_player.update_from_game(
                                10)  # تحديث جزئي أثناء اللعب
                    ...
                    if check_win(self.board, self.current_player):
                        ...
                        # تحديث نهائي للـ AI بعد المباراة
                        if self.ai_player:
                            reward = 100 if self.current_player == PlayerType.OPPONENT else -100
                            self.ai_player.update_from_game(reward)
                            self.ai_player.save_data()
