...
from sticks import throw_sticks, grants_extra_turn
...
class SenetGame:
    ...
    def _get_player_choice(self, valid_moves):
        ...
        while choice is None:
            try:
                user_input = input(f"  {c.BOLD}Select move index:{c.RESET} ")
                c_idx = int(user_input)
                if 0 <= c_idx < len(valid_moves):
                    choice = valid_moves[c_idx]
                else:
