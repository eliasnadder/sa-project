...
class Evaluation:
    ...
    def evaluate_board(self, board,valid_moves=None):
        ...
        if valid_moves: total_score += self._evaluate_flexibility(valid_moves)
...
#___________________________________________________  
    ...
    def _evaluate_special_houses(self, board):
        ...
        elif board[HOUSE_WATER] == self.opponent: score += abs(self.weights['water'])
        
        elif board[HOUSE_REBIRTH] == self.player: score -= 100
...
#______________________________________
    ...
    def _is_protected(self, board, pos):
        if pos > 0 and board[pos - 1] == board[pos]: return True 
        if pos < BOARD_SIZE - 1 and board[pos + 1] == board[pos]: return True
        return False
...
#______________________________________
    ...
    def evaluate_move(self, board, move, roll):
        ...
        if to_pos == OFF_BOARD: score += self.weights['piece_off']
            ...
            elif from_pos == HOUSE_THREE_TRUTHS and roll == 3: score +=300
            elif from_pos == HOUSE_THREE_TRUTHS and roll == 3 and to_pos != OFF_BOARD: score -=150

        
            elif from_pos == HOUSE_RE_ATUM and roll == 2: score +=200
            elif from_pos == HOUSE_RE_ATUM and roll == 2 and to_pos != OFF_BOARD: score -=150 

            elif from_pos == HOUSE_OF_HAPPINESS and roll == 4: score += 90
            elif from_pos == HOUSE_HORUS and roll in [1,2,3,4,5] and to_pos != OFF_BOARD: score-= 200
